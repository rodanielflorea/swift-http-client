...
public struct URLSessionTransport: ClientTransport {
  ...
  public struct Configuration: Sendable {
    ...
    public init(session: URLSession = .shared) {
      self.init(session: session, implementation: .platformDefault)
    }
    ...
    public struct HTTPBodyProcessingMode: Sendable {
      ...
      private init(_ implementation: Configuration.Implementation) {
    ...
    enum Implementation {
      case buffering
      case streaming(
        requestBodyStreamBufferSize: Int,
        responseBodyStreamWatermarks: (low: Int, high: Int)
      )
    }
    ...
    init(session: URLSession = .shared, implementation: Implementation = .platformDefault) {
      ...
      if case .streaming = implementation {
        precondition(
          Implementation.platformSupportsStreaming,
          "Streaming not supported on platform"
        )
      }
      self.implementation = implementation
    }
  ...
  fileprivate init(name: Name, isoLatin1Value: String) {
    if isoLatin1Value.isASCII {
      ...
      self = withUnsafeTemporaryAllocation(
        of: UInt8.self,
        capacity: isoLatin1Value.unicodeScalars.count
      ) {
        buffer in
        ...
        try await withCheckedThrowingContinuation {
          ...
          let task = self.dataTask(with: urlRequest) { [urlRequest] data, response, error in
            ...
            guard let response else {
              continuation.resume(
                throwing: URLSessionTransportError.noResponse(url: urlRequest.url)
              )
              return
            }
          ...
          taskBox.withLockedValue { boxedTask in
            guard task.state == .suspended else {
              debug(
                "URLSession task cannot be resumed, probably because it was cancelled by onCancel."
              )
              return
            }
            task.resume()
